...
namespace SharpWeb.Browsers
{
    ...
    public class AesGcm
    {
        public byte[] Decrypt(byte[] key, byte[] iv, byte[] aad, byte[] cipherText, byte[] authTag)
        {
            ...
            BCRYPT_AUTHENTICATED_CIPHER_MODE_INFO authInfo = new BCRYPT_AUTHENTICATED_CIPHER_MODE_INFO(iv, aad, authTag);
            ...
            if (status != ERROR_SUCCESS)
                throw new CryptographicException(
                    $"BCryptDecrypt() (get size) failed with status code: {status}");
            ...
            if (status != ERROR_SUCCESS)
                throw new CryptographicException($"BCryptDecrypt() failed with status code:{status}");
        ...
        {
            uint status = BCryptOpenAlgorithmProvider(out var hAlg, alg, provider, 0x0);

            if (status != ERROR_SUCCESS)
                throw new CryptographicException(
                    $"BCryptOpenAlgorithmProvider() failed with status code:{status}");
            ...
            if (status != ERROR_SUCCESS)
                throw new CryptographicException(
                    $"BCryptSetAlgorithmProperty(BCRYPT_CHAINING_MODE, BCRYPT_CHAIN_MODE_GCM) failed with status code:{status}");
        ...
        private IntPtr ImportKey(IntPtr hAlg, byte[] key, out IntPtr hKey)
        {
            ...
            if (status != ERROR_SUCCESS)
                throw new CryptographicException($"BCryptImportKey() failed with status code:{status}");
        ...
        private byte[] GetProperty(IntPtr hAlg, string name)
        {
            ...
            if (status != ERROR_SUCCESS)
                throw new CryptographicException(
                    $"BCryptGetProperty() (get size) failed with status code:{status}");
            ...
            if (status != ERROR_SUCCESS)
                throw new CryptographicException($"BCryptGetProperty() failed with status code:{status}");
